...
class BranchesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create branch" do
    assert_difference('Branch.count') do
      post branches_url, params: { branch: { name: @branch.name, repository_id: @branch.repository_id } }
    end
  ...
  test "should update branch" do
    patch branch_url(@branch), params: { branch: { name: @branch.name, repository_id: @branch.repository_id } }
