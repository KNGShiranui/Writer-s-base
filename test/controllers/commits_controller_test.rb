...
class CommitsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create commit" do
    assert_difference('Commit.count') do
      post commits_url, params: { commit: { branch_id: @commit.branch_id, message: @commit.message, user_id: @commit.user_id } }
    end
  ...
  test "should update commit" do
    patch commit_url(@commit), params: { commit: { branch_id: @commit.branch_id, message: @commit.message, user_id: @commit.user_id } }
